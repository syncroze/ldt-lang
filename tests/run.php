...
// --- scalar --------------------------------------------------------------
check('block set + interp', 'testing', Ldt::render('[set a]testing[/set]@{a}'));
check('self-closing set + interp', 'testing', Ldt::render('[set a = testing]@{a}'));
check('embedded mid-text', 'consectetur', Ldt::render('conse[set a = testing]ctetur'));
check('interp splices mid-word', 'consectetur', Ldt::render('[set a = sec]con@{a}tetur'));
check('interp mid-text', 'constestingequat', Ldt::render('[set a = testing]cons@{a}equat'));
...
check('quoted value may interpolate', '<  Ada  >', Ldt::render('[set n = Ada][set g = "  @{n}  "]<@{g}>'));
...
throws('the old or-fallback syntax is gone (loud)', fn () => Ldt::render('@{missing or "x"}'));
...
check('escaped ] in an unquoted value', 'see [1] here', Ldt::render('[set c = see [1\] here]@{c}'));
check('bare trailing slash needs no escape', 'http://x.com/', Ldt::render('[set url = http://x.com/]@{url}'));
check('value stops at the first unescaped ]', '<x>y]', Ldt::render('[set a = x]<@{a}>y]'));
...
errorAt('lex error in a multiline block value reports its real line', 3, 1,
    fn () => Ldt::render("[set r]\ngood\n@{bad..path}\n[/set]@{r}"));
errorAt('error inside a quoted = value reports template coords', 1, 11,
...
// --- block values are mini-templates ---------------------------------------
...
throws('tag opener in the = form fails loudly', fn () => Ldt::render('[set a = x [if @b]y[/if]]'));
...
// --- literals & the freed-up bracket -------------------------------------
check('lone [ is literal', 'a [ b', Ldt::render('a [ b'));
check('doubled [[ is now literal (no interp collision)', '[[x]]', Ldt::render('[[x]]'));
check('bracket flush against interp is fine now', '[hi]', Ldt::render('[set x=hi][@{x}]'));
...
check('escaped backslash then interp', '\\' . 'hi', Ldt::render('[set x=hi]\\\\@{x}'));
...
// --- expressions: comparison / logic render as 1 / empty -----------------
...
check('escaped @( is literal', '@(1 + 2)', Ldt::render('\@(1 + 2)'));
check('bare @ before ( with no expr intent stays literal via escape', 'a@(b', Ldt::render('a\@(b'));
...
throws('closed @{ } rejected inside @( )', fn () => Ldt::render('[set a=1]@(@{a} + 1)'));
...
throws('closed @{ } ref is rejected inside a tag', fn () => Ldt::render('[set a = x][if @{a}]yes[/if]'));
check(
...
throws('closed @{ } iterable is rejected inside a header', fn () => Ldt::render('[set b. = a][set b. = c][for v in @{b}]@{v}[/for]'));
...
// --- default for undefined: @{ path or fallback } ------------------------
...
check('default catches a false @() result', 'no', Ldt::render('[set age=15]@(@age >= 18 | default: "no")'));
...
// --- filters: in @() and expression args -------------------------------------
check('filter in @()', '6', Ldt::render('@(2 * 3 | abs)'));
...
check('array ref filtered inside @()', 'a+b', Ldt::render('[set i.=a][set i.=b]@(@i | join: "+")'));
...
throws('unterminated interp', fn () => Ldt::render('@{a'));
